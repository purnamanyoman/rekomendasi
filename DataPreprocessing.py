    ...
    while i<len_data_points and j<len_data_points:
        ...
        while j<len_data_points:
            ...
            if(max_len-count>2):
                ...
                new_data_list.append(data_set['web-scraper-order'].loc[row_wise_str_index[j]])
                if(max_len-1==j):
        ...
        if temp_i == i:
            break   
    data_set=data_set.loc[data_set['web-scraper-order'].isin(new_data_list)]
    return data_set
...
def RemoveDuplicateItemsNonAdjacent(data_set): # Time complexity = O(n^2) 
    ...
    while(len(row_wise_str_index)!=0):
        ...
        new_data_list.append(data_set['web-scraper-order'].loc[i])
